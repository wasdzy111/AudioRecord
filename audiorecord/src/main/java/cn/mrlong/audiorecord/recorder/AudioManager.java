...
public class AudioManager {
    ...
    public void prepareAudio() {
        try {
            ...
            // 设置文件音频的输出格式为wav
            // mRecorder.setOutputFormat(AudioFormat.CHANNEL_CONFIGURATION_MONO);
            // 设置音频的编码格式为amr
            mRecorder.setAudioEncoder(MediaRecorder.AudioEncoder.AMR_NB);
            // 设置音频的编码格式为wav(请修改后面的文件名)
    ...
    public int getVoiceLevel(int maxLevel) {
        ...
        if (isPrepared && null != mRecorder) {
            try {
                ...
                return level;
                // 取证+1，否则去不到7
//                Logger.i(maxLevel * mRecorder.getMaxAmplitude() / 32768 + 1+"当前");
